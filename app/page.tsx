...
export default function Home() {
  ...
  useEffect(() => {
    // Check for data from email page
    const storedEmail = sessionStorage.getItem("userEmail")
    const storedCalculationData = sessionStorage.getItem("calculationData")
    const storedIsReturningUser = sessionStorage.getItem("isReturningUser")

    if (storedEmail) {
      setUserEmail(storedEmail)

      if (storedCalculationData && storedCalculationData !== "null") {
        const calculationData = JSON.parse(storedCalculationData)
        setLoadedCalculation(calculationData)
        setShowWelcomeMessage(true)
        setIsReturningUser(storedIsReturningUser === "true")
      } else {
      ...
      sessionStorage.removeItem("isReturningUser")
    }
